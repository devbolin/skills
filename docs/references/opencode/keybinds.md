...
OpenCode uses a `leader` key for many keybinds. By default, `ctrl+x` is the leader key.

`leader_timeout` controls how long OpenCode waits for the next key after the leader key. Defaults to `2000` milliseconds.

---

## Binding Values

A string can contain one shortcut or multiple comma-separated shortcuts. You can also use an array or an object with `key`, `event`, `preventDefault`, or `fallthrough`.
...
The OpenCode desktop app supports common Readline/Emacs-style shortcuts: `ctrl+a`, `ctrl+e`, `ctrl+b`, `ctrl+f`, `alt+b`, `alt+f`, `ctrl+d`, `ctrl+k`, `ctrl+u`, `ctrl+w`, `alt+d`, `ctrl+t`, `ctrl+g`.
